...
| Backtest harness design (gate #6) | Built (brain `c311f35`, `backtest.py`) — replays in-play filter → §4.3b tells → signal_engine/ORB entry → stop/target/time-stop/EOD exit bar-by-bar using the SAME production modules live trading calls, not a re-implementation. `regime_detector`/`signal_engine` gained an optional `now` override (brain `c311f35`) so historical bars replay through the real intraday-clock gates instead of the wall clock. Runs on the Mac. Decision-fidelity interface: `backtest.decision_fidelity_replay()`. **Still blocked on data**: needs historical candles — nothing in the harness substitutes for having the data | Gate #6 needs the same code path replayed, not a lookalike, or decision-fidelity (§6.2) is meaningless; harness existing separately from data access means the gate can close the moment data does |
| Historical data source (gate #6) | Kite Connect (₹500/mo, no static IP needed for data-only calls — confirmed against Zerodha's current docs). Considered: TradingView (no official bulk API, only ToS-violating scrapers), Fyers (genuinely free, but same 100-day-per-request chunking as Kite — no real edge once building a chunked puller anyway), NSE bhavcopy/yfinance (free but daily-only, insufficient for an intraday strategy) | Zero new integration work (`kite_client.py` already has `get_historical_data()`), and it's the same auth path §3c already plans for live trading — one signup eventually serves both |
| Historical data scope + storage (gate #6) | Pull the full available range (walk backward until the API returns empty — gate 6's "≥ 2020/2021/2022" is a floor, marginal cost of more years is near-zero once the puller exists), not just the three named regime windows. Land it as local Parquet/CSV on the Mac, not the live Supabase `candles` table | Full-range data is basically free once chunked pagination is built anyway; the live table is sized for paper-session archiving, not a multi-year multi-symbol research dataset — mixing the two risks cost/quota problems and conflates backtest data with live data |
