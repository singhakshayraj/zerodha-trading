...
has no proven edge** — paper PF ≈ 0.33 over the 3 measured sessions; that's a
...
## Deployed versions
- **Brain:** `4e2230113d7a` (GIT_SHA set; deploy with `zerodha-brain/scripts/deploy.sh`).
- **Dashboard:** auto-deploys from `main` on Vercel. CI gates both repos on push.

## The 3 measured sessions (edge evidence)
...
Cumulative −58.9R; tape SIDEWAYS→BEARISH. **Standing conclusion: no edge yet →
gate #6 is the priority.** Trade-quality first read (T4): the **opening hour is
the only +EV window**; stops are blown through (STOP_LOSS_HIT avg −1.59R). Detail
in [ROADMAP.md](ROADMAP.md) T4.
...
- `trading_sessions.git_sha` = `4e2230113d7a`, not `unknown`.
