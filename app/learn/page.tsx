...
const GLOSSARY: { slug: string; term: string; short: string; body: string }[] = [
  ...
  { slug: "calibration", term: "Calibration / ECE", short: "Does 70% confidence mean right 70% of the time?",
    body: "Expected Calibration Error measures the gap between stated confidence and actual hit rate. Measured on 98 graded calls in August 2026, this system's confidence number turned out to carry NO information at all: it averaged 70.3 when the call was right and 69.8 when it was wrong, and its AUC was 0.492 where 0.5 is a coin flip — 0.513 when the same test is run market-neutral, against how the stock did versus the index rather than in absolute terms. Neither separates right from wrong. Worth knowing why ECE alone would have hidden that — a signal that always guesses the base rate is perfectly calibrated and completely useless. Calibration asks 'is 70% honest?'; discrimination asks 'does the number separate right from wrong at all?'. Here it does not, so confidence is displayed and never fed into a decision." },
